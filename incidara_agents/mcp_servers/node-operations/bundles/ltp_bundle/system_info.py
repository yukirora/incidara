...
try:
    ...
    if os.getenv('SB_SYS_INFO_STANDALONE_RUN') != '1':
        raise RuntimeError(
            'Failed to import superbench logger, and SB_SYS_INFO_STANDALONE_RUN not enabled'
        )
    import logging
...
class SystemInfo():    # pragma: no cover
    ...
    def get_system(self):
        ...
        try:
            ...
            # 38 - IPMI/BMC Device Information in SMBIOS
    ...
    def get_memory(self):
        ...
        try:
            ...
            memory_dict['memory_devices'] = []
            dmidecode_memory = self._run_cmd('dmidecode --type memory')
            dmidecode_memory_devices = [x.splitlines() for x in dmidecode_memory.split('Memory Device')[1:]]
            for memory_device_info in dmidecode_memory_devices:
                model = self._parse_key_value_lines(
                    memory_device_info, ['Manufacturer', 'Part Number', 'Type', 'Speed', 'Configured Memory Speed', 'Volatile Size'],
                    omitted_values=['other', 'unknown', 'no dimm', 'none']
                )
                if model:
                    memory_dict['memory_devices'].append(model)
        except Exception:
        ...
        try:
            ...
            for nic in lsnic_list:
                ...
                try:
                    ...
                    configuration_dict = {c.get('@id'): c.get('@value', '') for c in configuration}
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Output path for system info JSON'
    )
    args = parser.parse_args()

    system_info_collector = SystemInfo()
    system_info = system_info_collector.get_all()
