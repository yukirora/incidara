...
if [ -d deb ] && ls deb/containerd.io_*.deb >/dev/null 2>&1; then
  ...
  # Install IPMICFG if bin/ exists
  if [ -d bin ] && [ -f bin/IPMICFG-Linux.x86_64 ]; then
    cd bin
    USER=$(whoami)
    sudo install -m 0755 -o "${USER}" -g "${USER}" IPMICFG-Linux.x86_64 /usr/local/bin/IPMICFG-Linux.x86_64 || true
    cd ..
  fi
else
